...
class EightPuzzle(tk.Tk):
    ...
    def __init__(self, icon = None, state = default):
        ...
        self.solution = tk.Label(self, text=self.solver.solve(self.state))
        self.solution.grid(row = 3, columnspan = 3)
        
    def findLabel(self, state, label):
        for i in range(3):
            for j in range(3):
                if state[i][j] == label:
                    return i, j
    ...
    def move(self, state, row, col):
        row_e, col_e = self.findLabel(state, 0)
    ...
    def search(self):
        msg = self.solver.solve(self.state)
        if msg == "":
            msg = "Solved"
